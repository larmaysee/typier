...
export interface LayoutPerformanceResult {
  ...
  recommendations: LayoutAnalysisRecommendation[];
}
...
export interface LayoutAnalysisRecommendation {
...
export class AnalyzeLayoutPerformanceUseCase {
  ...
  async execute(
    command: AnalyzeLayoutPerformanceCommand
  ): Promise<LayoutPerformanceResult> {
    // Get layout information
    const layout = await this.keyboardLayoutRepository.findById(
      command.layoutId
    );
    if (!layout) {
  ...
  private async getLayoutAnalytics(
    ...
    const layoutAnalytics = allAnalytics.filter(
      (a) =>
    ...
    const averageWpm = this.calculateAverage(
      layoutAnalytics.map((a) => a.data.averageWpm)
    );
    const averageAccuracy = this.calculateAverage(
      layoutAnalytics.map((a) => a.data.accuracy)
    );
  ...
  private async generateLayoutComparison(
    ...
    const primaryAnalytics = await this.getLayoutAnalytics(
      userId,
      primaryLayoutId,
      timeRangeInDays
    );
    const primaryLayout = await this.keyboardLayoutRepository.findById(
      primaryLayoutId
    );
  ...
  private async generateRecommendations(
    analytics: LayoutAnalytics,
    layout: KeyboardLayout
  ): Promise<LayoutAnalysisRecommendation[]> {
    const recommendations: LayoutAnalysisRecommendation[] = [];
    ...
    if (analytics.problemKeys.length > 0) {
      ...
      recommendations.push({
        ...
        description: `You have high error rates on keys: ${topProblemKeys
          .map((k) => k.key)
          .join(", ")}`,
    ...
    if (layout.isCustom) {
      const layoutScore = this.calculateLayoutEfficiencyScore(
        analytics,
        layout
      );
      if (layoutScore < 70) {
  ...
  private calculateImprovementRate(analytics: TypingAnalytics[]): number {
    ...
    const daysDiff =
      (new Date(
        sortedAnalytics[sortedAnalytics.length - 1].generatedAt
      ).getTime() -
        new Date(sortedAnalytics[0].generatedAt).getTime()) /
      (1000 * 60 * 60 * 24);
  ...
  private analyzeFingerUtilization(
    analytics: TypingAnalytics[]
  ): FingerUtilization {
    // Aggregate keystroke data across all analytics
    const allKeystrokes = analytics.flatMap(
      (a) => a.data.keystrokePattern || []
    );
    ...
    const totalKeystrokes = Object.values(fingerCounts).reduce(
      (sum, count) => sum + count,
      0
    );
    ...
    Object.entries(fingerCounts).forEach(([finger, count]) => {
      fingerDistribution[finger] =
        totalKeystrokes > 0 ? (count / totalKeystrokes) * 100 : 0;
    });
    ...
    const leftHand = leftFingers.reduce(
      (sum, finger) => sum + (fingerDistribution[finger] || 0),
      0
    );
    const rightHand = rightFingers.reduce(
      (sum, finger) => sum + (fingerDistribution[finger] || 0),
      0
    );
  ...
  private identifyProblemKeys(analytics: TypingAnalytics[]): ProblemKey[] {
    const keyProblems: Record<
      string,
      { totalErrors: number; totalTime: number; count: number; finger: string }
    > = {};

    analytics.forEach((a) => {
      (a.data.keystrokePattern || []).forEach((keystroke) => {
        if (!keyProblems[keystroke.key]) {
          keyProblems[keystroke.key] = {
            totalErrors: 0,
            totalTime: 0,
            count: 0,
        ...
        keyProblems[keystroke.key].totalErrors +=
          keystroke.errorRate * keystroke.frequency;
        keyProblems[keystroke.key].totalTime +=
          keystroke.averageTime * keystroke.frequency;
    ...
    return Object.entries(keyProblems)
      .map(([key, data]) => ({
        ...
        finger: data.finger,
        suggestions: this.generateKeySuggestions(
          key,
          data.totalErrors / data.count
        ),
      }))
  ...
  private identifyStrengthAreas(analytics: TypingAnalytics[]): StrengthArea[] {
    ...
    const consistentHighAccuracy = analytics.filter(
      (a) => a.data.accuracy > 95
    );
  ...
  private calculateLayoutEfficiencyScore(
    analytics: LayoutAnalytics,
    layout: KeyboardLayout
  ): number {
  ...
  private generateComparisonRecommendations(
    ...
    if (primary && best.layoutId !== primaryLayoutId) {
      recommendations.push(
        `Consider switching to ${best.layoutName} for better overall performance`
      );

      if (best.averageWpm > primary.averageWpm) {
        recommendations.push(
          `${best.layoutName} shows ${(
            best.averageWpm - primary.averageWpm
          ).toFixed(1)} WPM improvement`
        );
      }
      ...
      if (best.averageAccuracy > primary.averageAccuracy) {
        recommendations.push(
          `${best.layoutName} shows ${(
            best.averageAccuracy - primary.averageAccuracy
          ).toFixed(1)}% better accuracy`
        );
      }
    }
