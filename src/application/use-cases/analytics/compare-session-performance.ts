...
export class CompareSessionPerformanceUseCase {
  ...
  async execute(
    command: CompareSessionPerformanceCommand
  ...
  private generateComparisonSummary(
    ...
    const worstSession = {
      ...
      highlightMetric: "WPM",
      highlightValue:
        sessionScores[sessionScores.length - 1].session.data.averageWpm,
    };
    ...
    const lastDate = new Date(
      sessionsData[sessionsData.length - 1].generatedAt
    );
    ...
    return {
      ...
      averageMetrics: {
        ...
        consistency: this.calculateMetricsSummary(
          sessionsData.map((s) => s.data.progressTrend?.[0]?.value || 0)
        ),
        errorRate: this.calculateMetricsSummary(
          sessionsData.map((s) => 100 - s.data.accuracy)
        ),
      },
      improvementRate,
      consistencyScore,
  ...
  private generateComparisonRecommendations(
    ...
    const strongSessions = sessions.filter(
      (s) => s.relativePerformance.percentileRank > 75
    );
    const weakSessions = sessions.filter(
      (s) => s.relativePerformance.percentileRank < 25
    );
    ...
    if (strongSessions.length > 0) {
      ...
      if (commonFactors.length > 0) {
        recommendations.push({
          ...
          description:
            "Your best sessions share common characteristics that can be replicated",
    ...
    if (weakSessions.length > 0) {
      ...
      recommendations.push({
        ...
        description:
          "Focus practice on areas where performance consistently drops",
        rationale: `Analysis shows recurring issues in ${problemAreas.join(
          ", "
        )}`,
        expectedImpact: "Improved baseline performance",
        actionSteps: problemAreas.map(
          (area) => `Dedicated practice for ${area}`
        ),
  ...
  private analyzeTrends(sessionsData: TypingAnalytics[]): TrendAnalysis {
    ...
    metrics.forEach((metric) => {
      ...
      trendMetrics.push({
        metric,
        direction:
          trend.slope > 0.01 ? "up" : trend.slope < -0.01 ? "down" : "stable",
        rate: trend.slope,
    ...
    const overallSlope =
      trendMetrics.reduce((sum, tm) => sum + tm.rate, 0) / trendMetrics.length;
    const overallTrend =
      overallSlope > 0.01
        ? "improving"
        : overallSlope < -0.01
    ...
    for (let i = 0; i < metrics.length; i++) {
      for (let j = i + 1; j < metrics.length; j++) {
        ...
        correlations.push({
          ...
          strength: this.interpretCorrelationStrength(
            Math.abs(correlation.coefficient)
          ),
          ...
          examples: this.generateCorrelationExamples(
            sessionsData,
            metric1,
            metric2,
            values1,
  ...
  private calculateMetricsSummary(values: number[]): {
    avg: number;
    min: number;
    max: number;
    stdDev: number;
  ...
  private calculateRelativePerformance(
    ...
    const scores = allSessions.map((s) =>
      this.calculateCompositeScore(s, 0, 0)
    );
    ...
    const percentileRank =
      ((allSessions.length - rank + 1) / allSessions.length) * 100;

    // Calculate deviations from average
    const avgWpm = this.calculateAverage(
      allSessions.map((s) => s.data.averageWpm)
    );
    const avgAccuracy = this.calculateAverage(
      allSessions.map((s) => s.data.accuracy)
    );
  ...
  private identifyStandoutFeatures(
    session: TypingAnalytics,
    averageMetrics: MetricsSummary
  ): StandoutFeature[] {
    ...
    const wpmDeviation =
      (session.data.averageWpm - averageMetrics.wpm.avg) /
      averageMetrics.wpm.stdDev;
    if (Math.abs(wpmDeviation) > 1.5) {
      features.push({
        metric: "WPM",
        value: session.data.averageWpm,
        significance: wpmDeviation > 1.5 ? "excellent" : "concerning",
        description: `${Math.abs(wpmDeviation).toFixed(
          1
        )} standard deviations ${wpmDeviation > 0 ? "above" : "below"} average`,
        context: "Speed performance",
      });
    }

    const accuracyDeviation =
      (session.data.accuracy - averageMetrics.accuracy.avg) /
      averageMetrics.accuracy.stdDev;
    if (Math.abs(accuracyDeviation) > 1.5) {
      features.push({
        metric: "Accuracy",
        value: session.data.accuracy,
        significance: accuracyDeviation > 1.5 ? "excellent" : "concerning",
        description: `${Math.abs(accuracyDeviation).toFixed(
          1
        )} standard deviations ${accuracyDeviation > 0 ? "above" : "below"
          } average`,
  ...
  private detectAnomalies(sessions: SessionAnalysis[]): ComparisonInsight[] {
    ...
    const outliers = sessions.filter(
      (s) =>
        s.relativePerformance.percentileRank > 90 ||
        s.relativePerformance.percentileRank < 10
    );
    ...
    outliers.forEach((outlier) => {
      insights.push({
        type: "anomaly",
        title:
          outlier.relativePerformance.percentileRank > 90
            ? "Exceptional Performance"
            : "Performance Dip",
        description: `Session ${outlier.sessionId} shows ${outlier.relativePerformance.percentileRank > 90
            ? "exceptional"
            : "concerning"
          } performance`,
        evidence: [
  ...
  private detectImprovementPatterns(
    sessionsData: TypingAnalytics[]
  ): ComparisonInsight[] {
    const insights: ComparisonInsight[] = [];
    ...
    if (trend.slope > 0.5) {
      insights.push({
        ...
        description:
          "Your typing speed shows consistent improvement across sessions",
        evidence: [
          {
            metric: "wpm",
  ...
  private calculateTrendMetric(values: number[]): {
    slope: number;
    acceleration: number;
    significance: number;
  } {
    if (values.length < 3)
      return { slope: 0, acceleration: 0, significance: 0 };
    ...
    const residualSumSquares = values.reduce(
      (sum, val, i) => sum + Math.pow(val - predictedY[i], 2),
      0
    );
  ...
  private generateProjections(
    ...
    trendMetrics.forEach((tm) => {
      const currentValue =
        sessionsData[sessionsData.length - 1].data.averageWpm; // Simplified
  ...
  private analyzeVolatility(
    ...
    return {
      ...
      byMetric: {
        wpm: wpmVolatility,
        accuracy: this.calculateVolatility(
          sessionsData.map((s) => s.data.accuracy)
        ),
      },
      patterns: [
        {
