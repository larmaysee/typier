...
export class AdaptDifficultyUseCase {
  async execute(userPerformance: UserPerformanceData): Promise<AdaptDifficultyResult> {
    // Analyze user performance trends
    const performanceTrend = this.analyzePerformanceTrend(userPerformance.recentSessions);
    ...
    const reasoning = this.generateReasoning(competencyLevel, performanceTrend, errorAnalysis);
    const adjustments = this.generateAdjustments(userPerformance, recommendedDifficulty);
  ...
  private analyzePerformanceTrend(sessions: SessionData[]): 'improving' | 'stable' | 'declining' {
    if (sessions.length < 3) return 'stable';

    // Get last 5 sessions for trend analysis
    const recentSessions = sessions.slice(-5);
    const wpmTrend = this.calculateTrend(recentSessions.map(s => s.wpm));
    const accuracyTrend = this.calculateTrend(recentSessions.map(s => s.accuracy));
  ...
  private assessCompetencyLevel(userPerformance: UserPerformanceData): 'beginner' | 'intermediate' | 'advanced' {
  ...
  private analyzeErrors(weakAreas: WeakArea[]): 'high' | 'medium' | 'low' {
    ...
    const averageErrorRate = weakAreas.reduce((sum, area) => sum + area.errorRate, 0) / weakAreas.length;
  ...
  private generateReasoning(
    ...
    if (errorLevel === 'high') {
      reasoning += "I recommend focusing on accuracy with an easier difficulty level.";
    } else if (errorLevel === 'low') {
      reasoning += "you're ready for more challenging content to continue growing.";
    } else {
      reasoning += "maintaining current difficulty with minor adjustments would be optimal.";
    }
  ...
  private generateAdjustments(
    ...
    const recommendedWordCount = this.getRecommendedWordCount(recommendedDifficulty);
