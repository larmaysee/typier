...
const DATABASE_ID = process.env.NEXT_PUBLIC_APPWRITE_DATABASE_ID || 'typoria-db';
const COLLECTIONS = {
  ...
  collections: [
    ...
    },
    {
      ...
      attributes: [
        ...
        { key: 'preferred_language', type: 'enum', elements: ['english', 'lisu', 'myanmar'], required: true, default: 'english' },
      ...
      indexes: [
        { key: 'user_id_settings_idx', type: 'key', attributes: ['user_id'] }
      ]
    }
  ]
};
...
${TYPORIA_DATABASE_CONFIG.collections.map(collection => `
   📊 Collection: ${collection.name}
   ID: ${collection.id}
   Permissions: ${collection.permissions.join(', ')}
   
   Attributes:
${collection.attributes.map(attr => `     - ${attr.key} (${attr.type}${attr.size ? `, size: ${attr.size}` : ''}${attr.required ? ', required' : ''}${attr.default !== undefined ? `, default: ${attr.default}` : ''}${attr.elements ? `, elements: [${attr.elements.join(', ')}]` : ''})`).join('\n')}
...
4. Verify your .env.local file contains:
   ...
   NEXT_PUBLIC_APPWRITE_COLLECTION_TYPING_TESTS=your_typing_tests_collection_id
   NEXT_PUBLIC_APPWRITE_COLLECTION_LEADERBOARDS=your_leaderboards_collection_id
   NEXT_PUBLIC_APPWRITE_COLLECTION_USER_SETTINGS=your_user_settings_collection_id
...
export function validateEnvironmentVariables(): { isValid: boolean; missing: string[]; usingDefaults: string[] } {
  const required = [
    ...
    'NEXT_PUBLIC_APPWRITE_COLLECTION_TYPING_TESTS',
    'NEXT_PUBLIC_APPWRITE_COLLECTION_LEADERBOARDS',
    'NEXT_PUBLIC_APPWRITE_COLLECTION_USER_SETTINGS'
  ];
  ...
  const usingDefaults = [];
  if (!process.env.NEXT_PUBLIC_APPWRITE_DATABASE_ID) usingDefaults.push('DATABASE_ID (default: typoria-db)');
  if (!process.env.NEXT_PUBLIC_APPWRITE_COLLECTION_USERS) usingDefaults.push('COLLECTION_USERS (default: users)');
  if (!process.env.NEXT_PUBLIC_APPWRITE_COLLECTION_TYPING_TESTS) usingDefaults.push('COLLECTION_TYPING_TESTS (default: typing_tests)');
  if (!process.env.NEXT_PUBLIC_APPWRITE_COLLECTION_LEADERBOARDS) usingDefaults.push('COLLECTION_LEADERBOARDS (default: leaderboards)');
  if (!process.env.NEXT_PUBLIC_APPWRITE_COLLECTION_USER_SETTINGS) usingDefaults.push('COLLECTION_USER_SETTINGS (default: user_settings)');
...
export function generateMCPSetupCommands(): string[] {
  ...
  // Create database
  commands.push(`mcp_appwrite-api_databases_create --database_id="${DATABASE_ID}" --name="${TYPORIA_DATABASE_CONFIG.database.name}"`);

  // Create collections and attributes
  for (const collection of TYPORIA_DATABASE_CONFIG.collections) {
    // Create collection
    commands.push(`mcp_appwrite-api_databases_create_collection --database_id="${DATABASE_ID}" --collection_id="${collection.id}" --name="${collection.name}"`);
...
export function getCurrentDatabaseConfig() {
  return {
    ...
    isConfigured: !!process.env.NEXT_PUBLIC_APPWRITE_ENDPOINT && !!process.env.NEXT_PUBLIC_APPWRITE_PROJECT_ID,
    environment: {
      endpoint: process.env.NEXT_PUBLIC_APPWRITE_ENDPOINT,
      projectId: process.env.NEXT_PUBLIC_APPWRITE_PROJECT_ID,
      databaseId: process.env.NEXT_PUBLIC_APPWRITE_DATABASE_ID || '(using default)',
      collections: {
        users: process.env.NEXT_PUBLIC_APPWRITE_COLLECTION_USERS || '(using default)',
        typingTests: process.env.NEXT_PUBLIC_APPWRITE_COLLECTION_TYPING_TESTS || '(using default)',
        leaderboards: process.env.NEXT_PUBLIC_APPWRITE_COLLECTION_LEADERBOARDS || '(using default)',
        userSettings: process.env.NEXT_PUBLIC_APPWRITE_COLLECTION_USER_SETTINGS || '(using default)'
      }
    }
  };
}