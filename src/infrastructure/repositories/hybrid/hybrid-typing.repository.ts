...
export class HybridTypingRepository implements ITypingRepository {
  ...
  async save(test: TypingTest): Promise<void> {
    ...
    // Check if user is a guest or anonymous
    const isGuest = !test.userId || test.userId === "anonymous" || test.userId.startsWith("guest_");

    // Try to save to Appwrite, but don't fail if offline
    // Only sync if it's a real user (not guest) AND not practice mode
    try {
      if (test.mode !== "practice" && !isGuest) {
      ...
      // Queue for retry when online (but only for non-practice modes and real users)
      if (test.mode !== "practice" && !isGuest) {
  ...
  async bulkSave(tests: TypingTest[]): Promise<void> {
    ...
    const nonPracticeTests = tests.filter((test) => {
      const isGuest = !test.userId || test.userId === "anonymous" || test.userId.startsWith("guest_");
      return test.mode !== "practice" && !isGuest;
    });
  ...
  private async processSyncQueue(): Promise<void> {
    try {
      ...
      if (failedOperations.length !== queue.length) {
        ...
        this.logger.info(
          `Sync completed: ${successfulOperations.length} successful, ${failedOperations.length} remaining`
        );
      }
    } catch (error) {
